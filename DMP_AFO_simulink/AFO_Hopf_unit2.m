function AFO_Hopf_unit2(block)
% level2 s-function of adaptive Hopf oscillator unit in Cartesian coordinate
% input: F(t) (pertubation term) 
% output: phi, omega
% parameters: K,  x0, y0, omega0, mu
%   Author: Tang kai
...
% Register parameters
block.NumDialogPrms     = 5; % parameters: K,  x0, y0, omega0, mu
...
% parameters: K,  x0, y0, omega0, mu
%%Continuous states
block.ContStates.Data(1) = block.DialogPrm(2).Data; %x_0
block.ContStates.Data(2) = block.DialogPrm(3).Data; %y0
...
block.OutputPort(1).Data =  atan2(y, x); % phi
...
r_square = x^2 + y^2;
% update dx
block.Derivatives.Data(1) = (mu - r_square)*x - omega*y + K * F_t;  
% update dy
block.Derivatives.Data(2) = (mu - r_square)*y + omega*x;
% update dOmega
block.Derivatives.Data(3) = -K*F_t* y / sqrt(r_square);
