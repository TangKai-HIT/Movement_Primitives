...
model.kV = (2*model.kP)^0.5; %Damping gain (with ideal underdamped damping ratio)
...
xTar = demos{1}.pos(:,end); %goal point
Data=[];
DataDMP=[];
for n=1:nbSamples
...
X = ones(nbSamples*nbData,1); %use piecewise constant function 
Y = DataDMP';
for i=1:model.nbStates
	W = diag(H2(i,:));
	MuF(:,i) = (X' * W * X \ X' * W * Y)';
